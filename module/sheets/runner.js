export default class cbrRunner extends foundry.appv1.sheets.ActorSheet {
    ...
    async actionRoll(){
        ...
        if (dataRoll.GLITCHED) {
            rollResult.slice(0,dataRoll.GLITCHED).forEach( dice => {
                if (dice <= 3) {
                    templateData.desc += `<div class="GLITCHED">${dice}: It triggers a further <strong>Level 2 Consequence</strong> that canâ€™t be Resisted.</div>`;
                }
    ...
    async overLOAD() {
        const templateData = {
            ...
            img: this.actor.img,
            name: this.actor.name,
            desc: "<p>When you mark your last âš¡, an Approach of your choosing becomes permanently <em>GLITCHED</em> and immediately incurs a complication, such as:</p>" +
            "<ul><li>You are taken out of action.</li><li>You expose the teamâ€™s weakness.</li><li>You make the team lose an advantage.</li><li>You damage or overlook something crucial.</li></ul>"+
            "<p>While <strong>OVERLOADED</strong> you canâ€™t perform actions that would require âš¡. <strong>TAKE A BREATHER</strong> to relieve âš¡ and clear this status.</p>",
            action: "",
            approach: "OVERLOAD"
        };
        const content = await renderTemplate('systems/cbrpnk-sop/templates/roll-card.hbs', templateData);
    ...
    async resistRoll() {
        ...
        if (rollResult.filter( dice => dice == 6).length == 2) {
            ...
            templateData.class = "critical";
            templateData.desc = "THATâ€™S IT.";
        }
        else if (Math.max(...rollResult) == 6 ) {
            stress += 1;
            templateData.title = "SOLID";
            templateData.class = "good";
            templateData.desc = "MARK âš¡";
            ...
            templateData.class = "bad";
            templateData.desc = "MARK âš¡âš¡âš¡";
    ...
    async breathRoll() {
        ...
        if (rollResult.filter( dice => dice == 6).length == 2) {
            templateData.title = "YOUâ€™VE BEEN THROUGH WORSE.";
            templateData.class = "critical";
            templateData.desc = "<strong>CLEAR ALL âš¡</strong> -OR- <strong>IGNORE BOTH</strong> LEVELS 1 AND 2 HARM PENALTIES FOR THE REMAINDER OF THE RUN.";
        }
        else if (Math.max(...rollResult) == 6 ) {
            templateData.title = "THAT SHOULD WORK.";
            templateData.class = "good";
            templateData.desc = "<strong>CLEAR âš¡âš¡âš¡</strong> -OR- <strong>IGNORE</strong> LEVEL 1 OR 2 HARM PENALTIES FOR THE REMAINDER OF THE RUN.";
        }
        else if (rollResult.filter( dice => dice == 4 || dice == 5).length ) {
            templateData.title = "SUCK IT UP.";
            templateData.class = "consequence";
            templateData.desc = "<strong>CLEAR âš¡âš¡</strong> -OR- <strong>IGNORE</strong> LEVEL 1 HARM PENALTIES FOR THE REMAINDER OF THE RUN.";
        }
        else {
            templateData.title = "TAKE WHAT YOU CAN GET.";
            templateData.class = "bad";
            templateData.desc = "CLEAR âš¡";
            this.actor.update({ 
                "system.stress.value": Math.max( this.actor.system.stress.value - 1, 0),
                "system.stress.isLOAD": false
            });
